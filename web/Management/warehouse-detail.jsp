...
<head>
    ...
    <style>
        ...
        .main-img-frame img { width: 100%; height: 100%; object-fit: contain; }
        ...
        .btn-submit { background: #111827; color: white; width: 100%; padding: 12px; transition: 0.2s; border-radius: 50px; text-decoration: none; display: block; font-weight: 600; text-align: center;}
        .btn-submit:hover { background: #1f2937; color: white; transform: translateY(-1px); }
        .fc { font-size: 0.85rem; } 
        .fc .fc-button { padding: 0.3rem 0.6rem !important; }
    </style>
</head>
...
<body>
    ...
    <div class="layout">
        <%-- Xác định quyền hạn --%>
        <c:set var="isStaff" value="${sessionScope.user.role == 'Manager' || sessionScope.user.role == 'manager' || sessionScope.user.role == 'admin' || sessionScope.user.role == 'Admin'}" />
                ...
                <div class="row">
                    <div class="col-lg-8">
                        <div class="card-custom p-3 mb-4">
                            ...
                            <c:set var="mainUrl" value="${not empty images ? images[0].imageUrl : 'default.jpg'}" />
                            <c:forEach items="${images}" var="item">
                                <c:if test="${item.primary}"><c:set var="mainUrl" value="${item.imageUrl}" /></c:if>
                            </c:forEach>
                            <div class="main-img-frame mb-3 text-center border bg-light rounded">
                                <img src="${pageContext.request.contextPath}/resources/warehouse/image/${mainUrl}" id="displayImg" onerror="this.src='https://via.placeholder.com/800x400?text=No+Image'">
                            </div>
                            <div class="row g-2">
                                <c:forEach items="${images}" var="img">
                                    <div class="col-3 col-md-2">
                                        <div class="border rounded p-1" style="cursor: pointer;">
                                            <img src="${pageContext.request.contextPath}/resources/warehouse/image/${img.imageUrl}" class="thumb-img w-100" style="height: 60px;" onclick="changeImage(this.src)" onerror="this.src='https://via.placeholder.com/80x80'">
                                        </div>
                                    </div>
                                </c:forEach>
                            </div>
    ...
    <div class="d-flex justify-content-between align-items-center mb-3">
        <div>
            <h5 class="fw-bold mb-0">Floor Plan View 
                <span class="text-muted fs-6 fw-normal ms-2">(Click to view calendar)</span>
            </h5>
        </div>
            ...
            <div class="d-flex gap-2">
                <a href="${pageContext.request.contextPath}/warehouse/detail?action=addUnit&warehouseId=${w.warehouseId}" 
                   class="btn btn-sm btn-dark shadow-sm">
                    <i class="fa fa-plus-circle me-1"></i> Add New Storage Unit
                </a>
            </div>
        </c:if>
    </div>
    ...
    <div class="card-custom p-3 mb-3 bg-light border border-light">
        <form action="${pageContext.request.contextPath}/warehouse/detail" method="GET" class="row align-items-end g-2">
            ...
            <div class="col-md-3">
                <button type="submit" class="btn btn-primary btn-sm w-100 fw-bold"><i class="fas fa-search me-1"></i> Find Available</button>
            </div>
            <div class="col-md-1 text-end">
                <a href="${pageContext.request.contextPath}/warehouse/detail?id=${w.warehouseId}" class="btn btn-light btn-sm border w-100"><i class="fas fa-undo"></i></a>
            </div>
        </form>
    </div>
        ...
        <c:choose>
                                    ...
                                    <c:otherwise>
                                        <c:forEach items="${units}" var="u">
                                            <%-- ĐIỀU KIỆN HIỂN THỊ: Renter chỉ thấy unit active, Manager thấy tất cả --%>
                                                            ...
                                                            <div class="d-flex justify-content-between align-items-start mb-2">
                                                                <h6 class="fw-bold text-dark mt-1">
                                                                    ...
                                                                    <c:if test="${u.status == 0}">
                                                                        <span class="badge bg-danger ms-1" style="font-size: 0.65rem;">Hidden</span>
                                                                    </c:if>
                                                                </h6>
                                                                <c:if test="${isStaff}">
                                                                    <a href="${pageContext.request.contextPath}/warehouse/detail?action=editUnit&unitId=${u.unitId}&warehouseId=${w.warehouseId}" 
                                                                       class="btn btn-sm btn-outline-primary py-0 px-2" onclick="event.stopPropagation();">
                                                                        <i class="fas fa-edit fs-7"></i>
                                                                    </a>
                                                                </c:if>
                                                            </div>
                                                            <p class="small text-muted mb-3" style="min-height: 40px;">${u.description}</p>
                                                            <h5 class="fw-bold text-dark mb-0"><fmt:formatNumber value="${u.area}" /> <span class="fs-6 text-muted"> m²</span></h5>
                                                        </div>
                                                        <div class="mt-3 pt-3 border-top text-primary fw-bold">
                        ...
                        <div class="right-sidebar"> 
                            <div class="card-custom p-3 mb-3" id="calendarWidget" style="display: none;">
                                <div class="d-flex justify-content-between align-items-center mb-2 pb-2 border-bottom">
                                    ...
                                    <button class="btn btn-sm btn-light border text-muted" type="button" data-bs-toggle="collapse" data-bs-target="#collapseCalendar">Show/Hide</button>
                                </div>
                                <div class="collapse show" id="collapseCalendar">
                                    <div class="row g-2 mt-1 mb-3">
                                        <div class="col-6">
                                            <select id="selectMonth" class="form-select form-select-sm border-secondary fw-bold">
                                                <option value="01">Jan</option><option value="02">Feb</option><option value="03">Mar</option><option value="04">Apr</option><option value="05">May</option><option value="06">Jun</option><option value="07">Jul</option><option value="08">Aug</option><option value="09">Sep</option><option value="10">Oct</option><option value="11">Nov</option><option value="12">Dec</option>
                                            </select>
                                        </div>
                                        <div class="col-6"><select id="selectYear" class="form-select form-select-sm border-secondary fw-bold"></select></div>
                                    </div>
                            ...
                            <c:if test="${!isStaff}">
                                <div class="card-custom p-4 text-center">
                                    ...
                                    <a href="${pageContext.request.contextPath}/createRentRequest?id=${w.warehouseId}" class="btn-submit">Request Rent</a>
                                </div>
                            </c:if>
                        </div> 
                    </div> 
                </div> 
                <jsp:include page="/feedback.jsp"><jsp:param name="embedded" value="true" /></jsp:include>
            </div> 
        </div> 
    </div> 
